...
import {
	html, css, SlyvexDialog, askForPassword, SVX,
	formatForMachine, getLocalWallet
} from './slyvex-dialog.js';
const pass = "";
import {Wallet} from '@slyvex/wallet-worker';

class SlyvexSendDialog extends SlyvexDialog{
	...
	static get styles(){
		return [SlyvexDialog.styles, 
		css`
			.container{
				max-height:var(--slyvex-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'slyvex' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in SVX')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${SVX(dataFee)} SVX</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${SVX(fee)} SVX</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${SVX(totalAmount)} SVX</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] SVX) is too large.');
			msg = msg.replace('[n]', SVX(estimate.fee));
...
SlyvexSendDialog.define("slyvex-send-dialog");
