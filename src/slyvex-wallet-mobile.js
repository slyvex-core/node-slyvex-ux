import {
	html, css, FlowFormat, SlyvexWalletUI, dpc,
	baseUrl, SVX, renderPagination, buildPagination, paginationStyle,
...
} from './slyvex-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class SlyvexWalletMobile extends SlyvexWalletUI{
	...
	static get styles(){
		return [SlyvexWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--slyvex-wallet-header-padding, 5px 10px);
				margin:var(--slyvex-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--slyvex-pagination-box-padding, 10px 5px;);
			}

			.logo{
				width:30px;height:30px;/*background-color:#DDD*/
				display:var(--slyvex-wallet-header-logo-diaplay, initial);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--slyvex-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--slyvex-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.tab.selected{
				border-bottom-color:var(--slyvex-wallet-tab-active-border-color, var(--flow-primary-color));
			}
			...
			.address-input{
				...
				min-width:var(--slyvex-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--slyvex-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--slyvex-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--slyvex-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--slyvex-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--slyvex-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--slyvex-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--slyvex-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
	...
	render(){
		...
		let donationAddresses = [
			["Slyvex Devfund donations:", "slyvex:qzrq7v5jhsc5znvtfdg6vxg7dz5x8dqe4wrh90jkdnwehp6vr8uj7csdss2l7"],
			["Slyvex WebWallet donations:", "slyvex:qqe3p64wpjf5y27kxppxrgks298ge6lhu6ws7ndx4tswzj7c84qkjlrspcuxw"],
		]
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">SlyVeX WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the slyvex ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Slyvex Core SVX:</span> ${window.PWA_MODULES['@slyvex/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Slyvex Wallet Framework SVX:</span> ${window.PWA_MODULES['@slyvex/wallet']}</div>
							<div class="badge"><span is="i18n-span">Slyvex gRPC:</span> ${window.PWA_MODULES['@slyvex/grpc']}</div>
							<div class="badge"><span is="i18n-span">Slyvex gRPC Relay:</span> ${window.PWA_MODULES['@slyvex/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Slyvex UX SVX:</span> ${window.PWA_MODULES['@slyvex/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX SVX:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">SLYVEX FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] SVX</b> available.', SVX(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${SVX(inUseUTXOs.satoshis||0)} SVX</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="/resources/images/slyvex.png"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.slyvex.network/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatSVX(total)+' SVX':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatSVX(available)} SVX</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatSVX(pending)} SVX</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'slyvextest:abc'
