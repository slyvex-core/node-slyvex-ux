...
import {
	html, css, SlyvexDialog, SVX,
	formatForMachine, formatForHuman
} from './slyvex-dialog.js';

class SlyvexT9Dialog extends SlyvexDialog{
	...
	static get styles(){
		return [SlyvexDialog.styles, css`
			.container{
				...
				max-height:var(--slyvex-dialog-container-max-height, 600px);
			}
	...
	renderBody(){
		...
		let {inputLabel='Amount in SVX'} = this;
		return html`
	...
	open(args, callback){
		...
		this.inputLabel = args.inputLabel||i18n.t('Amount in SVX');
		this.show();
	}
    cancel(){
    	this.hide();
...
SlyvexT9Dialog.define("slyvex-t9-dialog");
