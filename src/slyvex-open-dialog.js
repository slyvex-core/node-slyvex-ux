import {html, css, SlyvexDialog, i18n, T} from './slyvex-dialog.js';
import {askForPassword} from "./wallet.js";
const pass = "";

class SlyvexOpenDialog extends SlyvexDialog{
	...
	static get styles(){
		return [SlyvexDialog.styles, css`
			.container{max-height:var(--slyvex-dialog-container-max-height, 600px)}
			:host([mode="create"]) .container{max-height:var(--slyvex-dialog-container-max-height, 500px)}
			:host([mode="init"]) .container{max-height:var(--slyvex-dialog-container-max-height, 200px)}
			:host([mode="recover"]) .container{max-height:var(--slyvex-dialog-container-max-height, 450px)}
			.buttons{justify-content:center;--slyvex-dialog-buttons-width:100%;}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to SlyVeX Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="/resources/images/slyvex.png" />
			</div>`}
...
SlyvexOpenDialog.define("slyvex-open-dialog");
