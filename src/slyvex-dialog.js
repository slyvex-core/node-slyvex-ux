...
export class SlyvexDialog extends BaseElement{
	...
	static get styles(){
		return [ScrollbarStyle, SpinnerStyle, css`
			:host{
				...
				position:var(--slyvex-dialog-position, absolute);
				top:0px;
				left:0px;
				width:100%;
				height:100%;
				background-color:var(--slyvex-dialog-bg-color);
			...
			.container{
				box-sizing:border-box;
				width:100%;
				height:var(--slyvex-dialog-container-height, calc(100% - 10px));
				background-color:var(--flow-background-color, #F00);
				z-index:1;
				border:var(--slyvex-dialog-container-border, 2px solid var(--flow-primary-color));
				border-radius:var(--slyvex-dialog-container-border-radius, 3px);
				max-width:var(--slyvex-dialog-container-max-width, 700px);
				max-height:var(--slyvex-dialog-container-max-height, 300px);
				margin:var(--slyvex-dialog-container-margin, 5px auto);
				padding:var(--slyvex-dialog-container-padding, 0px);
				position:relative;
			...
			:host([hideable]) .close-btn{
				display:var(--slyvex-dialog-container-close-btn-display, inline-block)
			}
			.heading{
			...
			[hidden]{display:none}
			.buttons{margin:var(--slyvex-dialog-buttons-margin, 10px auto);display:flex;width:var(--slyvex-dialog-buttons-width,90%);}
	...
	pushHistory(uid=""){
		...
		name = name.replace(/\-/g, "");
		let key = name.toLowerCase().replace(/(slyvex|dialog|mobile)/g, '')
