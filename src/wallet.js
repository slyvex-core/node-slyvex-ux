...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@slyvex/wallet-worker';
export const {Deferred, SVX, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.SlyvexConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("SlyvexUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initSlyvexFramework, Storage} = require("slyvex-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("slyvex-grpc-node");
*/
